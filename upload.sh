...
Repo=""
RepoList=("PrivateRepo" "PrivateRepo_test" "PrivateRepo_gray")
getRepo() {
    echo -e "\n"
    length=${#RepoList[@]}
...
TestVersionLineNumber=`grep -nE 'version.test.*=' Assembly.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" Assembly.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' Assembly.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" Assembly.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' Assembly.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' Assembly.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" Assembly.podspec
...
pod repo push ${Repo} Assembly.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
